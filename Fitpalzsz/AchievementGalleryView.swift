    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 16) {
                ...
                List {
                    ...
                    Section(header: Text("Achievements").foregroundColor(.white)) {
                        ForEach(XPSystem.shared.gallery.achievements.sorted(by: { $0.name < $1.name })) { ach in
                            AchievementRow(
                                achievement: ach,
                    ...
                    Section(header: Text("Badges").foregroundColor(.white)) {
                        ForEach(XPSystem.shared.gallery.badges.sorted(by: { $0.name < $1.name })) { badge in
                            BadgeRow(
                                badge: badge,
            ...
            .toolbar {
                ...
                print("Achievements loaded:", XPSystem.shared.gallery.achievements.count)
...
#Preview {
    AchievementGalleryView().environmentObject(FriendStore())
}
